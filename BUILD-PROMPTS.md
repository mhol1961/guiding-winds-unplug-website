...
**Goal:** Get an Astro 6.x project running with TypeScript, Tailwind v4, and shadcn/ui primitives wired up. Repo structure matches `TECH-SPEC.md` §2.

**Status:** ✅ Completed 2026-05-17 (commits `1056883`, `bbc8210`, `5c0f88b`). This block is kept as the original prompt for reference; the deviations encountered during execution are recorded below.

**Deviations from the original prompt (rationale in DECISIONS.md):**
- Astro 6.3 was installed (not 5.x — 6 is the current stable).
- Tailwind v4 wires in via `@tailwindcss/vite` (not `@astrojs/tailwind`, which targets v3).
- The DESIGN.md → Tailwind theme is generated by a local `scripts/design-export.mjs` because `@google/design.md` v0.1.1 only emits Tailwind v3 JSON. See DECISIONS.md ADR-015.
- The DESIGN.md CLI binary is `design.md`, not `designmd`. Scripts use the correct name.
- shadcn/ui CLI was NOT initialized in Phase 1 — deferred to Phase 2 where the actual primitives get built. Initializing without using anything would have added orphaned config.
...
Scaffold a new Astro 6.x TypeScript project in this directory called "guidingwinds-unplug". Use the official `npm create astro@latest` flow with these answers:
- TypeScript: strict
- Install dependencies: yes
- Initialize git: no (we'll use the existing repo)

Then add these integrations and dependencies:
- tailwindcss + @tailwindcss/vite (Tailwind v4 — wired via the Vite plugin, NOT the deprecated @astrojs/tailwind which is v3-only)
- @astrojs/sitemap
- @astrojs/check (devDependency, for typecheck)
- @astrojs/cloudflare adapter (TECH-SPEC §9)
- @astrojs/react (for islands)
- zod
- @google/design.md (devDependency, used for linting DESIGN.md)
- js-yaml (devDependency, used by scripts/design-export.mjs to parse DESIGN.md frontmatter)
- @fontsource-variable/fraunces + @fontsource-variable/inter (self-hosted variable fonts; avoids render-blocking Google Fonts <link>)
- shadcn/ui — initialize with the CLI in Phase 2 (NOT here), to avoid orphaned config

Create the folder structure exactly as specified in TECH-SPEC.md §2: src/pages, src/content, src/components/{ui,home,voyage,journal,shared}, src/layouts, src/lib/{ghl,schema,utils}, src/styles, src/actions, public/{media,og}.
...
- "design:export": "node scripts/design-export.mjs"

Configure astro.config.mjs with the Cloudflare adapter, Tailwind via vite plugin, sitemap integration (site URL guarded — fail or warn in prod if PUBLIC_SITE_URL is unset; fall back to localhost in dev).

Set up Tailwind v4 by importing tailwindcss inside src/styles/theme.css (generated from DESIGN.md). Use Tailwind's CSS-first config approach — no tailwind.config.js.

Create src/styles/global.css that imports the @fontsource-variable packages and the generated theme.css.

Create the .env.example file per TECH-SPEC.md §6.

Add .gitignore entries: node_modules, dist, .env, .env.local, .env.production, .astro, .wrangler, .dev.vars.
...
Skills to invoke: project-kickoff (already used to generate the foundation docs — refresh recall), design-md (for theme export), astro-builder (read references/astro-setup.md for scaffolding patterns).
...
- `npm run build` produces dist/ with sitemap-index.xml using the production URL
- Folder structure matches TECH-SPEC.md §2 exactly
- Commit: "feat: scaffold Astro 6 + Tailwind v4 + Cloudflare adapter"
```
